...
Select * from gxfindta.tcltsd where TSBIDOP=4600669
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT A.*,
       CASE
         ...
         ELSE 'SI'
       END AS EXISTE_EN_TCLTSB
FROM (SELECT MOVRRNBEP,
             MOVPGON,
             CASE
...
FROM (SELECT CONCAT('CALL PGM(GXFINPGM/PCLR028) PARM',(CONCAT ((CONCAT ((CONCAT ((CONCAT ((CONCAT (CONCAT ('("',OPIDLIQ),'" "')),AUTRRNBEP)),'" "')),AUTTRXFCHC)),'")'))) AS LLAMADO
      FROM (SELECT OPIDLIQ,
            ...
            WHERE AUTRRNBEP IN ('227803400616',
'227903890855')));
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*Invoca proceso de Replicación del DRCONBEP A TSWAUT */
CALL PGM(GXPGBPS/APAUT011) PARM('20220915' '225896438785')
