...
-------------------------------------------------------------------------------------
select * from GXFINDTA.TCLMOD; --Detalle Comisionamiento
select * from GXFINDTA.TCLTPC; --Tabla Temporal
select * from GXFINDTA.TCLMOV; --Tabla de Movimiento
select * from GXFINDTA.TCLMOD; --Detalle Comisionamiento
select * from GXFINDTA.TCLHTM; --Historico del Temporal
...
select * from GXOPERA.COMERPRB; 	--Tabla de Comercios de Prueba
...
select * from ATMCENTRE.DEBUGATMC; 		--- Log Debug ATMCENTRE
...
select * from GXBDBPS.ATMCDTA; 			--Tabla de ATM s
...
select * from GXDEBITO.TGEMBHAF;		--Historico de Embozados
select * from LIBDEBITO.TGEMBHAF;		--Historico de Embozados
select * from GXBDBPS.TDBIN; 			--Bines de Tarjetas de Debito
select * from GXBDBPS.TDBINDET;			--Tabla Extendida de Bines
select * from GXBDBPS.TMCTAAF;			--Tabla de Disponibles.
select * from GXBDBPS.TDCLIENTE;		--Tabla de Clientes Tarjetas de Debito.
select * from GXBDBPS.TDCTA;			--Tabla de numeros de cuenta Debito
select * from GXBDBPS.TDCTAAD;			--Tabla de adherentes Tarj. Debito
select * from GXBDBPS.TDEMNOTAR;		--Tabla de Tarjetas de Debito.
select * from GXBDBPS.TDEMIS;			--Tabla de Emisiones
...
En lo que respecta a la reingeniería las tablas que guardan el PAN de tarjetas son:
GXFINDTA.TSWAUT --Autorizador
GXFINDTA.TCLTPC --Temporal
GXFINDTA.TCLHTM –histórico Temporal
GXFINDTA.TCLMOV --Movimientos
Otras tablas del sistema actual

LIBDEBITO.DRCONBEP --Autorizador
GXOPERA.OPMOVI --Movimientos
Módulo de Promociones y Puntos

GXFINDTA.TFIGP –Generación de Puntos
GXAUTDTA.TAULOWS –Log del Autorizador
GXFINDTA.TPRLOG --Promociones
GXSWTDTA.AIT002
--Las tablas que se graban el PAN desde las funcionalidades de sistemas de Marcas son:
GXSWTDTA.AIT002 -- Incoming Visa (Replica de GXBDBPS.AIT002)
GXBDBPS.TAUTPAF--Autorizaciones rol Emisor
GXSWTDTA.TMIFDT--Matriz de marcas, detalle transacciones
GXSWTDTA.ABUICDTA--Archivo ABU para envío a Mastercard, rol Emisor
GXSWTDTA.MCTN70D--Transacciones compensadas DUAL – Rol Adquirente
GXSWTDTA.MCT464FRDR--Transacciones compensadas SMS – Rol Adquirente
-- Podrían pasarme un listado de tablas donde se guarda PAN completo de tarjeta correspondiente a sus células.
--Es necesarios enmascarar las mismas para cumplir con PCI ya que se hará revisiones on site.
--Las que tengo hasta el momento ok son, ya que se usan de muestras en auditorias:
gxbdbps/gclieaf
gxbdbps/tdcliente
gxbdbps/tmoviaf
gxbdbps/tcuotaf
...
gxbdbps/tmovpag
/*
 Te paso este listado inicial de las tablas que contienen número de tarjeta de igual manera seguiré verificando y te paso lo antes posible si encuentro más tablas.

Tablas existentes en Desarrollo y Preprod
GXTRJDTA.TMiAdh
GXTRJDTA.TMITOK
GXTRJDTA.TMiTjt
GXTRJDTA.TMIRAd
...
GXTRJDTA.TMiBHi
GXTRJDTA.TMIAUP
Tablas existentes en Desarrollo,testing y Preprod
GXTRJDTA.TATAuT
GXTRJDTA.TMOCUD
GXTRJDTA.TMOCUO
GXTRJDTA.TTCTrj
